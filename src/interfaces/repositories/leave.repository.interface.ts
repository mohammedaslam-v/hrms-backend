import { LeaveRequestRecord } from '../../domain/leave';
...
export interface ILeaveRepository {
  ...
  /**
   * The manager's whole reporting tree, walked recursively — a skip-level
   * manager sees their reports' reports too. Excludes the manager themselves,
   * so nobody can approve their own leave.
   */
  findReportingTree(managerId: number): Promise<
    { id: number; employeeCode: string; fullName: string; designation: string | null }[]
  >;
